...
export class BasicCreationComponent implements OnInit {
  ...
  fromEventClick() {
    const subscription = fromEvent(document, 'click').subscribe((event)=>{
      console.log(event);
    });
  ...
  unsubscribeClick() {
    ...
    this.subscription = new Subscription();//Criado novo subscription pois o unsubscribe destroi o subscription
  }
